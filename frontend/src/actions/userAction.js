...
import { CLEAR_ERROR } from "./restaurantAction";
...
export const register = (userData) => async (dispatch) => {
  try {
    ...
    return data.data.user; //useful if calling functn needs this data
  } catch (error) {
    dispatch({
...
export const resetPassword =(token,passwords) => async(dispatch) =>{
  try{
    ...
    const {data} = await axios.post(`/api/v1/users/resetPassword/${token}`,passwords,config)
    dispatch({
